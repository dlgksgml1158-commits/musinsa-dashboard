"""무신사 파트너센터 쿠폰 마케팅(프로모션 쿠폰 신청) 현황 동기화.

notion-github-dashboard 저장소가 Playwright 로그인으로 직접 수집한
쿠폰 프로모션 일정(계정별 참여정보 제외, 이미 정제된 공개 데이터)을
그대로 가져와 표시한다. 이 저장소에서는 로그인 인증을 하지 않는다.
"""
import json
import os
...
SOURCE_URL = (
    ...
    "notion-github-dashboard/main/data-b53e82ab173f/musinsa_coupon_promotions.json"
)
OUT_PATH = "data/musinsa_coupon_promotions.json"
    ...
    if not items and os.path.exists(OUT_PATH):
        try:
            with open(OUT_PATH, encoding="utf-8") as f:
                prev = json.load(f)
            if prev.get("items"):
                print("Fetched 0 items; keeping previous coupon promotions")
                return
        except Exception:
            pass
